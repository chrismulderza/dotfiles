" Original from https://github.com/nvim-zh/minimal_vim/blob/master/init.vim
" Try to keep everything in one file so that it remains portabe

set nocompatible "In case were on a really old version of Vim
set encoding=utf-8
...
" Try to detect if we are running in an 256 color terminal, otherwise fallback
" to 16 colors.
if &term =~ "256color" || &term == "screen-256color" || $COLORTERM == "gnome-terminal"
  set t_Co=256
else
  set notermguicolors
endif
...
" General tab settings. Use 2 spaces for a tab/indent. This is more YAML
" friendly.
...
set spelllang=en_gb,en_us  " Spell languages, prefer the Kings English first
...
" remember that augroup makes sure that the autocommand is not loaded twice,
" and calling autocmd! clears all existing commands in the augroup.
...
" This may potentially help where remoting into a system that has some weird
" keyboard map set as well.
nnoremap ; :
xnoremap ; :
